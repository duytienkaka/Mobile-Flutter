...
class _ShoppingListDetailScreenState extends State<ShoppingListDetailScreen> {
  ...
  void _openAddItemSheet() {
    showModalBottomSheet(
      ...
      builder: (_) =>
          _AddShoppingItemSheet(service: service, listId: widget.list.id),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final totalCount = service.items.length;
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: service.isLoading && !hasItems
            ...
            ? ListView(
                ...
                children: [
                  ...
                  _buildSectionHeader(
                    title: context.tr('Cần mua'),
                    count: pendingCount,
                    icon: Icons.shopping_basket_outlined,
                  ),
                  const SizedBox(height: 10),
                  if (toBuyGroups.isEmpty)
                    _buildEmptySection(context.tr('Không còn món nào cần mua')),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: (_processing || _isSyncedToPantry)
                          ? null
                          : _onConfirmBuy,
                      icon: _processing
                          ? const SizedBox(
                      ...
                      style: ElevatedButton.styleFrom(
                        backgroundColor: _isSyncedToPantry
                            ? AppColors.textMuted
                            : AppColors.success,
          ...
          Row(
            children: [
              Container(
                ...
                child: Text(
                  context.tr('$pendingCount cần mua'),
              ...
              Container(
                ...
                child: Text(
                  context.tr('$completedCount đã mua'),
                  style: AppTextStyles.caption.copyWith(
                    fontStyle: FontStyle.normal,
    ...
    return Row(
      children: [
        ...
        Container(
          ...
          child: Text(
            context.tr('$count'),
            style: AppTextStyles.caption.copyWith(fontStyle: FontStyle.normal),
          ),
        ),
      ],
    );
  ...
  Future<void> _onConfirmBuy() async {
    ...
    try {
      ...
      for (final item in toAdd) {
        final safeUnit = item.unit.trim().isEmpty ? 'pcs' : item.unit.trim();
        final safeQuantity = item.quantity <= 0 ? 1.0 : item.quantity;
      ...
      if (toAdd.isNotEmpty) {
        Navigator.of(
          context,
        ).pushReplacement(MaterialPageRoute(builder: (_) => PantryScreen()));
...
class _AddShoppingItemSheet extends StatefulWidget {
  ...
  const _AddShoppingItemSheet({required this.service, required this.listId});
...
class _AddShoppingItemSheetState extends State<_AddShoppingItemSheet> {
  ...
  Future<void> _submit() async {
    ...
    final planDate = widget.service.activeListId != null
        ? widget.service.lists
              .firstWhere((l) => l.id == widget.service.activeListId)
              .planDate
        : today;
