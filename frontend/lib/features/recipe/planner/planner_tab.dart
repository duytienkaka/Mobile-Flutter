...
class _PlannerTabState extends State<PlannerTab> {
	...
	bool _shoppingActionLoading = false;
	...
	Future<void> _generateShoppingFromPlanner({
		required PlannerShoppingMode mode,
	}) async {
		if (_shoppingActionLoading) return;
		setState(() => _shoppingActionLoading = true);
		try {
			await service.loadWeek(weekStart);
			final result = await service.generateShoppingListsForWeek(
				weekStart: weekStart,
				mode: mode,
			);
            await ShoppingService.instance.loadLists();

			if (!mounted) return;
			if (result.sourcePlanCount == 0) {
				showTopSnackBar(
					context,
					'Tuần này chưa có kế hoạch để sinh danh sách đi chợ.',
					isError: true,
				);
				return;
			}

			if (result.createdListCount == 0) {
				showTopSnackBar(
					context,
					'Không thể tạo danh sách đi chợ, vui lòng thử lại.',
					isError: true,
				);
				return;
			}

			final modeLabel = mode == PlannerShoppingMode.daily
				? 'theo ngày'
				: 'gộp tuần';
			showTopSnackBar(
				context,
				'Đã sinh $modeLabel: ${result.createdListCount} danh sách, ${result.createdItemCount} món.',
				isError: false,
			);
			await _openShoppingAfterGeneration(mode, result.firstCreatedListId);
		} catch (e) {
			if (!mounted) return;
			showTopSnackBar(
				context,
			...
			if (mounted) {
				setState(() => _shoppingActionLoading = false);
			}
		}
	}

	Future<void> _openShoppingAfterGeneration(
		PlannerShoppingMode mode,
		String? firstCreatedListId,
	) async {
		if (!mounted) return;
		await ShoppingService.instance.loadLists();
		final rootNav = Navigator.of(context, rootNavigator: true);
		if (mode == PlannerShoppingMode.weeklyMerged && firstCreatedListId != null) {
			final shoppingService = ShoppingService.instance;
			final initialIndex = shoppingService.lists.indexWhere((e) => e.id == firstCreatedListId);
			var list = initialIndex == -1 ? null : shoppingService.lists[initialIndex];
			if (list == null) {
				await shoppingService.loadLists();
				final refreshedIndex = shoppingService.lists.indexWhere((e) => e.id == firstCreatedListId);
				list = refreshedIndex == -1 ? null : shoppingService.lists[refreshedIndex];
			}
			if (!mounted) return;
			if (list != null) {
				final targetList = list;
				await rootNav.push(
					MaterialPageRoute(builder: (_) => ShoppingListDetailScreen(list: targetList)),
				);
				return;
			}
		}

		await rootNav.pushReplacement(
	...
	Widget _buildWeeklyAutoBanner() {
		...
		final disableActions = _weeklyActionLoading || _shoppingActionLoading;
			...
			child: Column(
				...
				children: [
					...
					Wrap(
						...
						children: [
							OutlinedButton.icon(
								onPressed: disableActions ? null : _openWeeklyPreferencesSheet,
							...
							ElevatedButton.icon(
								onPressed: disableActions ? null : () => _runWeeklyPlan(regenerateAuto: false),
							...
							ElevatedButton.icon(
								onPressed: disableActions
									? null
									: () => _generateShoppingFromPlanner(mode: PlannerShoppingMode.daily),
								icon: const Icon(Icons.shopping_bag_outlined, size: 18),
								label: const Text('Sinh list theo ngày'),
								style: ElevatedButton.styleFrom(
									foregroundColor: AppColors.textPrimary,
									disabledForegroundColor: AppColors.textMuted,
									backgroundColor: AppColors.primarySoft,
							...
							OutlinedButton.icon(
								onPressed: disableActions
									? null
									: () => _generateShoppingFromPlanner(mode: PlannerShoppingMode.weeklyMerged),
								icon: const Icon(Icons.view_week_outlined, size: 18),
								label: const Text('Gộp tuần 1 list'),
								style: OutlinedButton.styleFrom(
									foregroundColor: AppColors.textPrimary,
									disabledForegroundColor: AppColors.textMuted,
									backgroundColor: AppColors.surface,
